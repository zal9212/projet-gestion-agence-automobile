...
function admin_dashboard() {
    ...
    render_view('admin/dashboard', ['stats' => $stats, 'recent_reservations' => $recent_reservations]);
}
...
function admin_car_form() {
    ...
    render_view('admin/car_form', ['car' => $car, 'categories' => $categories]);
}
...
function admin_car_save() {
    ...
    $data = [
        $_POST['category_id'], $_POST['immatriculation'], $_POST['marque'], $_POST['modele'],
        $_POST['type_carburant'], $_POST['boite_vitesse'], $_POST['nb_places'],
        $_POST['prix_journalier'], $_POST['caution'], $image_principale, $_POST['status']
    ];
...
function admin_reservations() {
    check_admin();
    $pdo = get_pdo();
    $reservations = $pdo->query("SELECT r.*, u.nom, u.prenom, u.telephone, c.marque, c.modele, c.immatriculation FROM reservations r JOIN users u ON r.user_id = u.id JOIN cars c ON r.car_id = c.id ORDER BY r.id DESC")->fetchAll();
    render_view('admin/reservations', ['reservations' => $reservations]);
}
...
function admin_save_checkout() {
    ...
    $stmt->execute([$_POST['kilometrage_depart'], $_POST['niveau_carburant_depart'], $_POST['signature_base64'], $_POST['id']]);
...
function admin_print_contract() {
    ...
    $stmt->execute([$_GET['id']]);
    $reservation = $stmt->fetch();
    render_view('admin/contract_print', ['reservation' => $reservation]);
}
